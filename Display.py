...
class Display:
    ...
    def display_results(total_train_rewards, total_infer_rewards, losses, epoch, epsilon, epsilon_drop, softmax,
        ...
        ax0.set(xlabel="episodes (in hundreds)", ylabel="total rewards",
                title="Train Rewards (Average {0:.1f})".format(np.mean(train_to_plot)))
        ax1.plot(np.arange(len(total_infer_rewards)), total_infer_rewards, color="green", label="Inference rewards")
        ax1.set(xlabel="episodes", ylabel="total rewards",
                title="Inference Reward (Average {0:.1f})".format(np.mean(total_infer_rewards)))
        ...
        ax2.legend()
        fig.suptitle(
            "DDRL (epoch {0}): start_epsilon={1}, epsilon_drop={2}, softmax={3}, guided_exploration={4}".format(epoch,
                                                                                                                epsilon,
                                                                                                                epsilon_drop,
                                                                                                                softmax,
                                                                                                                guided_exploration))
    ...
    def check_action(reward):
        '''
        checks if the given reward indicates that the arrow was hit correctly
        a correct hit would result in a reward of 0 or 1
        :return: true if non negative reward, false otherwise
        '''
        if reward == -1:
            return False
        return True

    def display_env(self, arrows_generated, rewards_earned):
        '''
        generates a display of arrows that demonstrates the arrows generated
        and whether or not the agent hit them correctly
        :param arrows_generated: arrows generated by environment
        :param rewards_earned: rewards earned by the agent for each arrow
        :return:
        '''
        ...
        # create list of arrows to be drawm on screen that correspond to the arrows
        # created by environment
        for i in range(len(rewards_earned)):
        ...
        while run:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    run = False
            # check if first arrow is correct
            if arrows[first].arrow_y >= 450:
                correct = self.check_action(rewards_earned[first])
                if correct:
